/*
 * Replica of MMDVMHost's FMControl.cpp RX audio filter: a 3-stage cascaded
 ...
 * their bandwidth exactly matches what MMDVMHost's RX chain already imposes
 * on real repeated voice, rather than approximate it with a freshly-designed
 * filter of unverified equivalence.
 *
 * One deliberate deviation: MMDVMHost's IIRDirectForm1Filter applies a
 ...
 * MMDVMHost restoring level for its own downstream chain, which works on
